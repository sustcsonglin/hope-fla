...
@triton.jit
def parallel_square_delta_rule_bwd_kernel(
    k,
    k2,
    v,
    beta,
    ...
    for offset in range(0, (i_t + 1) * BT - BS, BS):
        ...
        b_A_square = tl.where(m_s[:, None], b_A * b_A * scale, 0)
        b_dv = tl.dot(tl.trans(b_do), b_A_square.to(b_do.dtype))
...
# todo: to make k2 already k * beta[:, None]. 
@triton.jit
def parallel_square_delta_rule_bwd_kernel_dk(
    q,
    k,
    k2,
    v,
    ...
    for offset in range(tl.cdiv(T, BS) * BS - BS, i_t * BT, -BS):
        ...
        # b_A_square = tl.math.exp2(tl.where(m_s[None, :], b_A, float("-inf")) * sm_scale - b_l[:, None])
        ...
        # [BQS, BKT]
        # b_dp = tl.dot(b_do, b_v)
        # b_dA = ((b_dp - b_delta[:, None]) * b_A_square * scale).to(b_q.dtype)
...
def chunk_transform_qk_fwd_kernel(
    ...
    b_qkT_square = tl.where(o_i[:, None] >= o_i[None, :], b_qkT * b_qkT * scale,0)
    b_o = tl.dot(b_qkT_square.to(b_v.dtype), b_v)
...
def chunk_transform_qk_bwd_kernel(
    ...
    b_A_square = tl.where(m_t, b_A_local * b_A_local * scale, 0)
    b_dv = tl.dot(tl.trans(b_A_square.to(b_do.dtype)), b_do)
    ...
    b_k_beta = (b_k * b_beta[:, None]).to(b_k.dtype)
    b_dqkT = 2 * b_dqkT * b_A_local * scale
...
def chunk_transform_qk_fwd_fn(q, k, v, beta, A, scale, BT, output_attentions):
    ...
    return q_new, k_new, o, A_local, None, None
...
class ParallelDeltaRuleFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, beta, scale):
        ...
        q_new, k_new, o, _,  _, _ = chunk_transform_qk_fwd_fn(q, k, v, beta, A, scale, BS, False)
        ...
        #     )
        # square_attn = torch.exp2(attn * scale * 1.4142135623730951 - L_new[..., None])
    ...
    def backward(ctx, do):
        ...
        A = fwd_prepare_T(k, beta, None,  None, True, BS)
        q_new, k_new, _, A_local, _, _ = chunk_transform_qk_fwd_fn(q, k, v, beta, A, scale, BS, True)
        ...
        parallel_square_delta_rule_bwd_kernel[grid](
            k=k_new,
            k2=k,
            v=v,
            beta=beta,
        ...
        parallel_square_delta_rule_bwd_kernel_dk[grid](
            q=q_new,
            k=k_new,
            k2=k,
            v=v,
...
def naive_delta_rule_parallel(q, k, v, beta, scale, BM=128, BN=64):
    ...
    A = A.masked_fill_(~torch.tril(torch.ones(l, l, device=q.device, dtype=torch.bool)), 0)
...
if __name__ == "__main__":
    ...
    ref_o = ((ref_attn.float()**2) * K**-0.5).to(v) @ v
    ...
    print(get_err_ratio(ref_o, o2), (ref_o-o2).abs().max())
    ...
    # o3 = (A.to(torch.float32) * scale).square(-1).to(v) @ v
    # o3.backward(do)
